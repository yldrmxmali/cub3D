...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/14 07:16:14 by beeren            #+#    #+#             */
/*   Updated: 2024/03/28 01:49:35 by beeren           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	validate_comma(t_cub3d *cub3d)
{
	...
	if (count != 2)
		(ft_putstr_fd(F_ELEMENT_VALUE, 2), free_all(cub3d));
}
...
static void	validate_red_color(t_cub3d *cub3d)
{
	...
	if (red_len == 0)
		(ft_putstr_fd(F_ELEM_RED_EMPTY, 2), free_all(cub3d));
	if (red_len > 3)
		(ft_putstr_fd(F_ELEM_RED_LEN, 2), free_all(cub3d));
	i = 0;
	while (i < red_len)
	{
		if (!ft_is_digit(red_string[i]))
			(ft_putstr_fd(F_ELEM_RED_DIGIT, 2), free_all(cub3d));
		i++;
	}
	red = ft_atoi(red_string);
	if (red < 0 || red > 255)
		(ft_putstr_fd(F_ELEM_RED_RANGE, 2), free_all(cub3d));
...
static void	validate_green_color(t_cub3d *cub3d)
{
	...
	if (green_len == 0)
		(ft_putstr_fd(F_ELEM_GREEN_EMPTY, 2), free_all(cub3d));
	if (green_len > 3)
		(ft_putstr_fd(F_ELEM_GREEN_LEN, 2), free_all(cub3d));
	i = 0;
	while (i < green_len)
	{
		if (!ft_is_digit(green_string[i]))
			(ft_putstr_fd(F_ELEM_GREEN_DIGIT, 2), free_all(cub3d));
		i++;
	}
	green = ft_atoi(green_string);
	if (green < 0 || green > 255)
		(ft_putstr_fd(F_ELEM_GREEN_RANGE, 2), free_all(cub3d));
...
static void	validate_blue_color(t_cub3d *cub3d)
{
	...
	if (blue_len == 0)
		(ft_putstr_fd(F_ELEM_BLUE_EMPTY, 2), free_all(cub3d));
	if (blue_len > 3)
		(ft_putstr_fd(F_ELEM_BLUE_LEN, 2), free_all(cub3d));
	i = 0;
	while (i < blue_len)
	{
		if (!ft_is_digit(blue_string[i]))
			(ft_putstr_fd(F_ELEM_BLUE_DIGIT, 2), free_all(cub3d));
		i++;
	}
	blue = ft_atoi(blue_string);
	if (blue < 0 || blue > 255)
		(ft_putstr_fd(F_ELEM_BLUE_RANGE, 2), free_all(cub3d));
...
void	validate_floor_color(t_cub3d *cub3d, char *line)
{
	...
	if (line[0] == 'F')
	{
		...
		if (*cub3d->config->floor_color->string == '\0')
			(ft_putstr_fd(F_ELEMENT_EMPTY, 2), free_all(cub3d));
